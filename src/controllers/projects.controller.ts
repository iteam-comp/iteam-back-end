...
export class ProjectsController {
  ...
  async findById(
    ...
    if (!project) return this.response.status(404);

    const mainParticipant = await this.userRepository.findById(
      project.mainParticipantId
    );

    const participants =
      project.subParticipants &&
      (await Promise.all(
        project.subParticipants?.map((participantId: string) =>
          this.userRepository.findById(participantId)
        )
      ));

    return { ...project, mainParticipant, participants };
  }
